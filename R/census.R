...
# this function has been developed by Niels Vullings
#' @rdname ts_dyads
#' @export
#' @description Functions with _evo in their name calculate characteristics of
...
ts_dyads_evo <- function(sims, net1, simtype = "notypespecified", forplot = TRUE) {
  ...
  df <- foreach::foreach(1:nsims, i = iterators::icount(), .combine = "rbind") %dopar% {
    ...
    stablet1 <- jumpst1 # use this for stable asymmetric ties
    ...
    table <- cbind(
      ...
      nrow(jump02),
      nrow(jump20)
    ) # bind the rownumbers to determine the census of the tie type
    ...
    colnames(table) <- c(
      ...
      "Null > Mutual",
      "Mutual > Null"
    ) # Rename the columns to match the dyad change type
    simnet <- NULL
  ...
  df <- foreach::foreach(1:nsims, x = iterators::icount(), .combine = "rbind") %dopar% {
    ...
    df <- foreach::foreach(a1 = 1:nrow(net1), i = iterators::icount(), .combine = "rbind") %:%
      ...
      foreach::foreach(a3 = 1:nrow(net1), k = iterators::icount(), .combine = "rbind") %do% {
        if (i > j & j > k) {
          data.frame(
            i = i, j = j, k = k, # (i>j & j>k ) determines with or without repititions [current = NO REP]
